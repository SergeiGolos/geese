...
program
  .name('geese-auto')
  .description('CLI tool for processing .geese files with AI-powered transformations (auto mode)')
  .version('1.0.0');
...
async function main(directory, options) {
  console.log(chalk.blue('🦢 Geese - AI-powered file processing tool (Auto Mode)'));
  ...
  // Auto-select .geese files (or let user choose if multiple)
  let selectedGeeseFiles = geeseFiles;
  ...
  if (geeseFiles.length > 1) {
    ...
    const { processAll } = await inquirer.prompt([
      {
        type: 'confirm',
        name: 'processAll',
        message: 'Process all .geese files?',
        default: true
      }
    ]);
    ...
    if (!processAll) {
      ...
      selectedGeeseFiles = files;
    }
  }
  
  if (selectedGeeseFiles.length === 0) {
    console.log(chalk.yellow('No .geese files selected for processing.'));
    return;
  }
  
  console.log(chalk.blue(`📋 Processing ${selectedGeeseFiles.length} .geese file(s)`));
  
  // Process each selected .geese file
  const allSessions = [];
  
  for (const geeseFile of selectedGeeseFiles) {
    ...
    try {
      ...
      // Process all target files automatically without interactive selection
